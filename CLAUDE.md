...
**`obtenerRitmo(mes)` compares cycles at the same point, not calendar months.** It answers "am I spending more or less than last month": it takes the day of the cycle today falls on (`Ciclos.diaDelCiclo()`) and sums the first N days of this cycle against the first N days of the previous one, both cut with `Ciclos.corteDelCiclo()`. Three things it deliberately leaves out: credits (as everywhere else), **carried installments** (`cuota_actual > 1` — they're a fixed weight from old purchases and were 65% of a real statement's total, which would bury the signal), and anything dated on or after the cutoff. Rows are compared as `'YYYY-MM-DD'` strings, which order like dates without building a `Date` per row.

The previous month comes from the **cycle calendar, not from the list of months with data**: if that cycle was never imported, `hayAnterior` is false and `dibujarRitmo()` hides the panel rather than reporting a −100% against an empty month. A closed month compares both cycles in full (28 vs 28) and `enCurso` flips the panel's wording to the past tense. `pct` is `null` when the previous cycle had no consumption in the stretch — the UI then shows only the arrow and the amount instead of dividing by zero.
...
`dibujarRitmo()` fills the `.panel-ritmo` block that sits under the closing panel (green `▼` when this cycle is running cheaper than the last one at the same point, red `▲` when it isn't). Unlike `dibujarCierre()` it **does** depend on the selected month, so it is called from `dibujarDashboard()` and follows the month selector. It carries the whole empty-state policy: no previous cycle imported → the panel is hidden outright; no base for a percentage → arrow and amount only. Both `.panel-cierre` and `.panel-ritmo` share the glass surface rule but are *not* bound by the cursor-spotlight IIFE, which only walks `.tarjeta, .bento-caja` — they get the static fallback position (see Styling).
