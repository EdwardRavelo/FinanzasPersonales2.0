...
**This has already drifted once, and it was invisible until someone squinted at a legend.** `PALETTES.*.textMuted` had fallen a few steps darker than `--text-muted`, which put every chart's legend and axis text at **3.83:1** on the dark surface and 3.95:1 on the light one — under the 4.5:1 that small text needs, and worst on the donut legend, which is the channel carrying the amounts. The pairs that must stay identical are `textMuted` ↔ `--text-muted`, `textMain` ↔ `--text-main`, `gold` ↔ `--gold`, and `surface` ↔ `--bg-base`. Verify a change rather than eyeballing it: `validate_palette.js` exports `contrast(a, b)`, so `contrast('#7d90a0', '#080c12')` settles it in one line. `toggleTema()` also writes an `!important` inline `background-color` on `<body>` with hardcoded hex values (`#e8eaee` / `#080c12`) that mirror `--bg-base` — and `PALETTES.*.surface` carries the same two values a fourth time, because Chart.js needs them to paint the gaps between marks.
