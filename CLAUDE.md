...
`obtenerRitmo()` measures **three** things over the same window and with the same filter — pesos, dólares and a movement count — so the percentage means the same thing wherever it appears. The ARS figures stay flat on the object (`actual` / `anterior` / `delta` / `pct`) because the ritmo panel was written against them; the other two arrive as `usd` and `movimientos` sub-objects and feed `dibujarNotasComparacion()`, which fills the `.nota-kpi` line under the USD and movement KPIs and follows the same `#ritmo-vs` month selector. **Each note is a sentence, not notation** — `Vas gastando 24% más que en agosto`, `Tuviste 4% más movimientos que en agosto`. Two earlier passes tried compact forms (`▲ 24,1% vs Ago`, then `▲ 24,1% · 54 vs 52 mov · Ago 2026`) and both read as something to decode; the second also invited the reader to reconcile 54 against the 60 printed above it, which is not a mistake — the KPI counts every row of the month, the note counts cycle movements up to the cut-off. The prose sidesteps that comparison entirely, and the exact figures plus that caveat live in the `title`. Rules that keep it readable: the percentage is rounded to a whole number; under 1% it says "prácticamente lo mismo" rather than "0%"; a zero base becomes "En febrero no hubo consumo para comparar" instead of a division; and the verb follows the cycle ("Vas gastando" while it is open, "Gastaste" once closed).
...
Making the note count *everything* so it matches its KPI is not the fix: at day 19 of a closed month that would sweep in carried installments, which carry dates from earlier months, and the count would stop scaling with the day of the cycle.
